...
class AddActivityView: UIView {
    ...
    private func configure() {
        let views: [UIView] = [addButton, titleLabel, visualEffect, nameTextField, divider,
                               goalTextField, paidSwitch, visualEffectSmall, moneyPerHourTextField, categoryPicker]
        ...
        paidSwitch.addTarget(self,
                             action: #selector(switchTapped),
                             for: .touchUpInside)

        nameTextField.addTarget(nil,
                                action: #selector(AddActivityViewActionHandler.nameTFchanged),
                                for: .editingChanged)
